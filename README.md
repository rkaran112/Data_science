...
- **`finbot.py`** — A Streamlit-based "stock analysis chatbot" prototype. It defines helper functions for pulling stock data via `yfinance` (price, SMA, EMA, RSI, MACD, institutional holders, news) intended to be wired up to an OpenAI chat model as callable "functions", plus a small Streamlit UI. The OpenAI function-calling wiring is commented out and the code has known issues (e.g. `calculate_RSI` uses `delta` before it's defined, `personal_finance` is referenced but never implemented, a typo in `plt.xlabel9(...)`). It does not run as-is.
...
- `finbot.py` does not run as-is: it calls an undefined `personal_finance` function, `calculate_RSI` references `delta` before assignment, there's a typo (`plt.xlabel9`), and the OpenAI function-calling logic is commented out/incomplete.
